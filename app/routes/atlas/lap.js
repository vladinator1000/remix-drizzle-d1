...
export function lapJv(dim, cost) {
  // input:
  // dim        - problem size
  ...
  // u          - dual letiables, row reduction numbers
  // v          - dual letiables, column reduction numbers

  // convert the cost matrix (old API) to a callback (new API)
  if (typeof cost === 'object') {
    let cost_matrix = cost
    cost = function (i, j) {
  ...
  const collist = new Int32Array(dim) // list of columns to be scanned in letious ways.
